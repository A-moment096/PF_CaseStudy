...
enum BOUNDCOND { BndPERIODIC,
                 BndCONST,
                 BndADIABATIC };
enum STENCILE { StencFIVE = 5,
                StencNINE = 9 };
...
public:
    ...
    void addEntry(WHICHPARA whichpara, int num) {
        ...
        case WHICHPARA::CON:
            for (auto &node : SimuNodes)
                node.Con_Node.addEntry(num);
            break;
        case WHICHPARA::PHSFRAC:
            for (auto &node : SimuNodes)
                node.Phs_Node.addEntry(num);
            break;
        case WHICHPARA::TEMP:
            for (auto &node : SimuNodes)
                node.Temp_Node.addEntry(num);
            break;
        case WHICHPARA::CUSTOM:
            for (auto &node : SimuNodes)
                node.Cust_Node.addEntry(num);
            break;
        default:
            break;
        }
    ...
    void deletEntry(WHICHPARA whichpara, int _Index) {
        ...
        case WHICHPARA::CON:
            for (auto &node : SimuNodes)
                node.Con_Node.deletEntry(_Index);
            break;
        case WHICHPARA::PHSFRAC:
            for (auto &node : SimuNodes)
                node.Phs_Node.deletEntry(_Index);
            break;
        case WHICHPARA::TEMP:
            for (auto &node : SimuNodes)
                node.Temp_Node.deletEntry(_Index);
            break;
        case WHICHPARA::CUSTOM:
            for (auto &node : SimuNodes)
                node.Cust_Node.deletEntry(_Index);
            break;
        default:
            break;
        }
...
#pragma omp parallel for collapse(2)
            for (auto &node : SimuNodes) {
                for (int _Index = 0; _Index < Num_Ent; ++_Index) {
                    ...
                    result = (l + r - 2 * c) / (StepX * StepX) + (f + b - 2 * c) / (StepY * StepY) + (u + d - 2 * c) / (StepZ * StepZ);
                    switch (whichpara) {
                    case WHICHPARA::CON:
...
#pragma omp parallel for collapse(2)
        for (auto &node : SimuNodes) {
            for (int _Index = 0; _Index < Num_Ent; ++_Index) {
                ...
                double resultx = (node.getNbhd(WHICHDIR::DirR)->getVal(whichpara, _Index) - node.getNbhd(WHICHDIR::DirL)->getVal(whichpara, _Index)) / (2.0 * StepX);
                double resulty = (node.getNbhd(WHICHDIR::DirB)->getVal(whichpara, _Index) - node.getNbhd(WHICHDIR::DirF)->getVal(whichpara, _Index)) / (2.0 * StepY);
                double resultz = (node.getNbhd(WHICHDIR::DirD)->getVal(whichpara, _Index) - node.getNbhd(WHICHDIR::DirU)->getVal(whichpara, _Index)) / (2.0 * StepZ);
...
void SimulationMesh::showNodesProp(WHICHPARA which, int _Index) { // which para, _Index of para
    if (which == WHICHPARA::CON)
        std::cout << "Concentration of " << SimuNodes.at(0).Con_Node.Entrys.at(_Index).Element << "\n";
    if (which == WHICHPARA::PHSFRAC)
        std::cout << "Order Parameter of " << _Index << " grain\n";
    if (which == WHICHPARA::CUSTOM)
        std::cout << "Custom value \n";
    for (long i = 0; i < Num_Nodes; i++) {
        std::cout << std::fixed << std::setprecision(10) << SimuNodes.at(i).getVal(which, _Index)
                  // SimuNodes.at(i).getLap(WHICHPARA::CON, _Index)
                  << " ";
        if (i % MeshX == MeshX - 1)
            std::cout << "\n";
        if (i % (MeshX * MeshY) == MeshX * MeshY - 1)
            std::cout << "\n";
    }
    std::cout << "-----------------------------------------------------------------\n"
              << std::endl;
...
std::string toVTK_Path(std::string _dirname) {
    // legal name check
    if (_dirname.empty())
        throw std::invalid_argument("_dirname can't be empty");
    for (auto _ch : _dirname) {
        if (std::isspace(_ch))
            throw std::invalid_argument("_dirname can't contain space, maybe use underscore");
    }
